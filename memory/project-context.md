...
- **Resiliencia UI & Manejo de Errores de API (Streamlit & LangChain)**: En `app.py` y `src/agent/claim_agent.py`, se asegura el reseteo inmediato del panel inferior al pulsar "🚀 Evaluar Siniestro" o cambiar de caso. Ante errores de API (ej. 429 cuota agotada o 401 clave inválida), el error detallado se captura en `assessment.api_error` y se visualiza explícitamente en el dashboard junto con el aviso del modo de contingencia (motor determinista).
